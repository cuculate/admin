<template>
    <div class="container-fluid row p15">
        <h3 class="col-md-12">Phương thức thanh toán</h3>
        <div class="col-md-2">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">
                        <button class="btn btn-primary btn-control btn-flat" @click="searchPayment">Tìm kiếm</button>
                        <router-link :to="{ name: 'payment-create' }"
                                     class="btn btn-primary btn-control btn-flat ml-1">Thêm phương thức thanh toán
                        </router-link>
                ...
                <div class="card-body p-0">
                    <table-form :payments="payments" @checkedPayments="checkedPayments"></table-form>
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            searchPayment() {
                ...
                let uri = query.length ? `http://localhost:8000/admin/payment?${query}` : `http://localhost:8000/admin/payment`;
            ...
            setData(response) {
                this.payments = response.data.data.payment.data;
                this.prev = response.data.data.payment.prev_page_url;
                this.next = response.data.data.payment.next_page_url;
            },
            ...
            deletePayments() {
                let vm = this;
                let uri = `http://localhost:8000/admin/payment/delete/${vm.listIds}`;
                let token = {headers: {Authorization: 'Bearer ' + vm.$store.state.token}}
        ...
        created() {
            let vm = this;
            let uri = 'http://localhost:8000/admin/payment';
